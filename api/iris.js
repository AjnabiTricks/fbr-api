...
const CONFIG = {
  ...
  TIMEOUT: 30000,
  CREDIT: 'https://t.me/AZ_Tricks'  // 👈 Credit added here
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Extract NTN from Reference No (if exists)
    let ntn = '';
    if (parsed['Reference No']) {
      // Reference No format: "3801900-7" se NTN extract karein
      const refMatch = parsed['Reference No'].match(/^(\d+)/);
      if (refMatch) {
        ntn = refMatch[1];
      }
    }

    // Create final response with NTN and Credit
    const finalData = {
      ...parsed,
      'NTN': ntn || 'Not Found'
    };
    ...
    res.status(200).json({
      ...
      cnic: cnic,
      data: finalData,
      credit: CONFIG.CREDIT  // 👈 Credit in response
    });
